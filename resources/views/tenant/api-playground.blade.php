...
<!-- Page Header -->
<div class="mb-12">
    <h1 class="text-4xl font-black text-white tracking-tight uppercase">API <span class="text-supabase-accent">Playground</span></h1>
    ...
    <div class="lg:col-span-1 space-y-8">
        ...
        <div class="bg-supabase-surface border border-supabase-border rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            <div class="absolute top-0 right-0 p-4">
                <span class="px-3 py-1 bg-supabase-accent/10 text-supabase-accent text-[8px] font-black uppercase tracking-widest border border-supabase-accent/20 rounded-full">Secure Node</span>
            </div>
            
            <h2 class="text-xs font-black text-white uppercase tracking-[0.2em] mb-6 flex items-center">
            ...
            <div class="space-y-6">
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">Secret API Key</label>
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">Webhook Endpoint</label>
            ...
            <div class="mt-8 p-4 bg-supabase-dark border border-supabase-border rounded-2xl flex items-start space-x-3">
                ...
                <p class="text-[9px] text-supabase-muted font-bold uppercase leading-relaxed tracking-tight">Security Alert: Rotate keys immediately if compromised via the <a href="{{ route('tenant.settings') }}" class="text-supabase-accent underline">Control Panel</a>.</p>
            </div>
        </div>
        ...
        <div class="bg-supabase-surface border border-supabase-border rounded-3xl p-8 shadow-2xl overflow-hidden group">
            <div class="flex items-center justify-between mb-8">
                <h2 class="text-xs font-black text-white uppercase tracking-[0.2em]">Android Bridge</h2>
                <div class="flex items-center space-x-2">
                    <div class="w-2 h-2 rounded-full {{ $androidAppConnected ? 'bg-green-500 animate-pulse' : 'bg-supabase-muted' }}"></div>
                    <span class="text-[8px] font-black uppercase text-white tracking-widest">{{ $androidAppConnected ? 'Active' : 'Offline' }}</span>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="p-4 bg-supabase-dark border border-supabase-border rounded-2xl group-hover:border-supabase-accent/30 transition-colors">
                    <p class="text-[8px] font-black text-supabase-muted uppercase tracking-widest mb-1">Latest Pulse</p>
                    <p class="text-xs font-black text-white uppercase">{{ $latestPaymentLog ? $latestPaymentLog->created_at->diffForHumans() : 'No activity logged' }}</p>
                </div>
                @if(!$androidAppConnected)
                    <div class="p-6 bg-amber-500/5 border border-amber-500/20 rounded-2xl">
                        <p class="text-[9px] text-amber-500 font-black uppercase tracking-widest mb-2 italic">Bridge Required</p>
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-supabase-surface border border-supabase-border rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <form id="createTransactionForm" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">Target Channel</label>
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">Magnitude (Rp)</label>
                        <div class="relative">
                            <span class="absolute text-supabase-muted font-black text-xs z-10" style="left: 16px; top: 50%; transform: translateY(-50%);">Rp</span>
                            <input type="text" id="amountDisplay" placeholder="100.000" class="sb-input font-mono font-black tracking-wider relative z-0 w-full" style="padding-left: 45px !important;" required oninput="formatRupiah(this)">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">Entity Identity</label>
                        <input type="text" name="customer_name" placeholder="John Doe" class="sb-input" required>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">External Reference</label>
                        <input type="text" name="external_id" placeholder="TEST-ORDER-001" class="sb-input">
                    </div>
                </div>
                <div class="md:col-span-2 pt-4">
                    <button type="submit" class="sb-button-primary !w-full !py-4 uppercase font-black tracking-[0.2em]">
        ...
        <div class="bg-supabase-surface border border-supabase-border rounded-3xl p-8 shadow-2xl">
            ...
            <form id="testWebhookForm" class="space-y-8">
                @csrf
                <div class="space-y-4">
                    <label class="text-[10px] font-black text-supabase-muted uppercase tracking-widest">Recipient Endpoint</label>
                    <div class="flex flex-col md:flex-row gap-4">
                        <input type="url" name="webhook_url" id="testWebhookUrl" placeholder="https://webhook.site/..." class="sb-input flex-1" required>
                        <button type="button" onclick="window.open('https://webhook.site', '_blank')" class="sb-button-secondary !w-auto !px-8 text-[10px] uppercase font-black whitespace-nowrap">Open Hook Site</button>
                    </div>
                    <p class="text-[8px] text-supabase-muted font-bold uppercase tracking-tighter">HMAC SHA256 signatures will be automatically computed for the payload.</p>
                </div>

                <button type="submit" class="sb-button-secondary !w-full !py-4 !bg-blue-500/10 !border-blue-500/20 !text-blue-500 hover:!bg-blue-500 hover:!text-white font-black uppercase tracking-[0.2em]">
...
document.getElementById('createTransactionForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.transaction.channel_type === 'qris' && data.transaction.qr_svg) {
                paymentInstruction = `
                    <div class="mt-4 p-6 bg-white rounded-2xl flex flex-col items-center justify-center w-full overflow-hidden shadow-inner">
                        ...
                        <p class="mt-4 text-xs font-bold text-gray-500 uppercase tracking-widest">Scan to pay</p>
                        <p class="text-lg font-black text-gray-900 mt-1">Rp ${parseFloat(data.transaction.unique_amount).toLocaleString('id-ID')}</p>
                    </div>
                `;
            } else if (data.transaction.account_number) {
                paymentInstruction = `
                    <div class="mt-4 p-4 bg-supabase-surface border border-supabase-border rounded-xl">
                        <p class="text-[8px] font-black text-supabase-muted uppercase mb-1">Transfer Destination</p>
                        <p class="text-sm font-black text-white">${data.transaction.provider} - ${data.transaction.account_number}</p>
                        ...
                        <div class="mt-3 p-2 bg-supabase-dark rounded">
                            ...
                            <p class="text-lg font-black text-supabase-accent">Rp ${parseFloat(data.transaction.unique_amount).toLocaleString('id-ID')}</p>
                        </div>
                    </div>
                `;
            }
            ...
            content.innerHTML = `
                <div class="p-6 bg-supabase-dark border border-supabase-border rounded-2xl space-y-4">
                    <div class="flex items-center justify-between">
                        <p class="text-[10px] font-black text-supabase-accent uppercase tracking-widest">Generation Successful</p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-4 bg-supabase-surface border border-supabase-border rounded-xl">
                            <p class="text-[8px] font-black text-supabase-muted uppercase mb-1">Magnitude</p>
                            <p class="text-xs font-black text-white">Rp ${parseFloat(data.transaction.amount).toLocaleString('id-ID')}</p>
                        </div>
                        <div class="p-4 bg-supabase-surface border border-supabase-border rounded-xl">
                            <p class="text-[8px] font-black text-supabase-muted uppercase mb-1">Status</p>
                            <p id="playgroundInvoiceStatus" class="text-xs font-black text-amber-500 uppercase">${data.transaction.status}</p>
                        </div>
                    </div>
                        ...
                        if (checkData.success && checkData.invoice.status !== 'pending') {
                            ...
                            if (statusEl) {
                                ...
                                if (checkData.invoice.status === 'paid' || checkData.invoice.status === 'success') {
                                    statusEl.className = 'text-xs font-black text-green-500 uppercase';
                                    ...
                                    if (wrapper) {
                                        wrapper.innerHTML = `
                                            <div class="mt-4 p-8 bg-green-500/10 border border-green-500/20 rounded-xl flex flex-col items-center justify-center text-center">
                                                ...
                                                <h3 class="text-sm font-black text-green-500 uppercase tracking-widest mb-1">Pembayaran Diterima</h3>
                                                <p class="text-[10px] text-supabase-muted uppercase font-bold tracking-widest">Dana telah diverifikasi oleh sistem Node.</p>
                                            </div>
                                        `;
                                    }
                                } else {
                                    statusEl.className = 'text-xs font-black text-red-500 uppercase';
                                }
                            }
                        }
                    } catch (e) {
            ...
            content.innerHTML = `<div class="p-4 bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-black uppercase tracking-widest rounded-xl">Generation Failed: ${data.message}</div>`;
        }
    } finally {
        ...
        content.innerHTML = `
            <div class="space-y-4">
                <div class="p-4 bg-supabase-dark border border-supabase-border rounded-2xl">
                    <p class="text-[8px] font-black text-supabase-muted uppercase mb-2">Endpoint Response</p>
