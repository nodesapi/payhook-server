        ...
        <div class="flex items-center space-x-4">
            <div class="px-4 py-2 bg-supabase-surface border border-supabase-border rounded-xl">
                <div class="flex items-center space-x-2">
                    ...
                    <span class="text-xs font-bold uppercase tracking-widest text-white">{{ strtoupper($tenant->mode) }}</span>
                </div>
            </div>
            @if($tenant->plan)
                <div class="px-4 py-2 bg-supabase-accent/10 border border-supabase-accent/20 rounded-xl">
                    <div class="flex items-center space-x-2 text-supabase-accent">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-widest">{{ $tenant->plan->name }}</span>
                    </div>
                </div>
            @endif
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
    ...
    <div class="bg-supabase-surface border border-supabase-border rounded-2xl p-6 relative overflow-hidden group shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <p class="text-xs font-bold text-supabase-muted uppercase tracking-widest mb-1">Total Revenue</p>
            <h3 class="text-2xl font-black text-white">Rp {{ number_format($stats['total_revenue'], 0, ',', '.') }}</h3>
        </div>
    </div>
    ...
    <div class="bg-supabase-surface border border-supabase-border rounded-2xl p-6 relative overflow-hidden group shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <p class="text-xs font-bold text-supabase-muted uppercase tracking-widest mb-1">Success Tx</p>
            <h3 class="text-2xl font-black text-white">{{ number_format($stats['success_transactions']) }}</h3>
        </div>
    </div>
    ...
    <div class="bg-supabase-surface border border-supabase-border rounded-2xl p-6 relative overflow-hidden group shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <p class="text-xs font-bold text-supabase-muted uppercase tracking-widest mb-1">Today</p>
            <h3 class="text-2xl font-black text-white" id="live-today">{{ number_format($stats['today_transactions']) }}</h3>
        </div>
    </div>
    ...
    <div class="bg-supabase-surface border border-supabase-border rounded-2xl p-6 relative overflow-hidden group shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <p class="text-xs font-bold text-supabase-muted uppercase tracking-widest mb-1">Channels</p>
            <h3 class="text-2xl font-black text-white">{{ $stats['active_channels'] }} / {{ $stats['total_channels'] }}</h3>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 xl:grid-cols-5 gap-8 mb-12">
    <div class="xl:col-span-3 bg-supabase-surface border border-supabase-border rounded-2xl p-8 shadow-2xl relative overflow-hidden group">
        ...
        <div class="relative z-10">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
                <div>
                    <h3 class="text-xl font-black text-white tracking-tighter">Transaction <span class="text-supabase-accent">Activity</span></h3>
                    <p class="text-[10px] font-bold text-supabase-muted tracking-[0.2em] mt-1 uppercase">Volume monitoring (Last 7 Days)</p>
                </div>
                <div class="flex items-center space-x-2">
                    <span class="w-3 h-3 rounded-full bg-supabase-accent shadow-[0_0_15px_rgba(251,191,36,0.5)] animate-pulse"></span>
                    <span class="text-xs font-black text-white tracking-widest">Live Flow</span>
                </div>
            </div>
    ...
    <div class="xl:col-span-2 bg-supabase-surface border border-supabase-border rounded-2xl p-6 shadow-2xl overflow-hidden relative">
        ...
        <div class="relative z-10 flex items-start justify-between gap-4 mb-6">
            <div>
                <h3 class="text-lg font-black text-white tracking-tight">Mobile App <span class="text-supabase-accent">Relay</span></h3>
                <p class="text-[10px] font-bold text-supabase-muted uppercase tracking-[0.2em] mt-1">Apps being watched on Android</p>
            </div>
            <div class="flex items-center gap-2 rounded-xl border border-supabase-border bg-supabase-dark/60 px-3 py-2">
                ...
                <span class="text-[9px] font-black uppercase tracking-widest text-white">{{ $listenerOnline ? 'Listening' : 'Standby' }}</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-[190px_1fr] xl:grid-cols-1 2xl:grid-cols-[190px_1fr] gap-6 items-start">
            <div class="mx-auto w-[190px] rounded-[2rem] border border-white/10 bg-[#0d0d0d] p-3 shadow-2xl">
                <div class="rounded-[1.5rem] border border-supabase-border bg-supabase-dark p-3 min-h-[330px] overflow-hidden">
                    <div class="mb-5 flex items-center justify-between text-[8px] font-black uppercase tracking-widest text-supabase-muted">
                        <span>9:41</span>
                    ...
                    <div class="rounded-2xl border border-supabase-border bg-supabase-input/80 p-3 mb-4">
                        <div class="flex items-center gap-3">
                            <div class="h-9 w-9 rounded-xl bg-supabase-accent text-supabase-dark flex items-center justify-center font-black text-xs">CB</div>
                            <div>
                                <p class="text-[10px] font-black text-white uppercase tracking-wider">Cekbayar</p>
                        ...
                        @foreach($mobileApps->take(6) as $channel)
                            ...
                            <div class="min-w-0 text-center">
                                <div class="relative mx-auto mb-2 h-12 w-12 rounded-2xl {{ $brand['tile'] }} {{ $type === 'qris' ? 'text-supabase-dark' : 'text-white' }} flex items-center justify-center border {{ $brand['ring'] }} shadow-lg">
                                    <span class="text-[11px] font-black tracking-tight">{{ $brand['mark'] }}</span>
                                ...
                                <p class="truncate text-[8px] font-black uppercase tracking-wider text-white">{{ $brand['label'] }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="space-y-3">
                @foreach($mobileApps->take(5) as $channel)
                    ...
                    <div class="flex items-center gap-4 rounded-2xl border border-supabase-border bg-supabase-dark/50 px-4 py-3">
                        <div class="h-11 w-11 flex-shrink-0 rounded-xl {{ $brand['tile'] }} {{ $type === 'qris' ? 'text-supabase-dark' : 'text-white' }} flex items-center justify-center font-black text-[11px] shadow-lg">
                            {{ $brand['mark'] }}
                        </div>
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                <p class="truncate text-xs font-black text-white">{{ $channelName }}</p>
                                <span class="h-1.5 w-1.5 flex-shrink-0 rounded-full {{ $isActive ? 'bg-green-500' : 'bg-supabase-muted' }}"></span>
                            </div>
                            <p class="truncate text-[9px] font-bold uppercase tracking-widest text-supabase-muted">{{ $provider }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs font-black text-white">{{ number_format($payloadCount) }}</p>
                            <p class="text-[8px] font-black uppercase tracking-widest text-supabase-muted">TX</p>
                        </div>
                    </div>
                @endforeach
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-supabase-surface border border-supabase-border rounded-2xl overflow-hidden shadow-2xl">
            <div class="px-8 py-6 border-b border-supabase-border flex items-center justify-between bg-supabase-surface/50">
                <div>
                    ...
                    <p class="text-xs text-supabase-muted uppercase tracking-widest mt-1">Live monitoring</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-supabase-dark/50 text-[10px] uppercase tracking-widest font-bold text-supabase-muted">
                        <tr>
                    ...
                    <tbody class="divide-y divide-supabase-border" id="live-invoice-list">
                        ...
                        <tr class="group hover:bg-white/[0.02] transition-colors">
                            ...
                            <td class="px-8 py-4">
                                <div class="text-sm font-black text-white">Rp {{ number_format($transaction->amount, 0, ',', '.') }}</div>
                            </td>
                            <td class="px-8 py-4">
    ...
    <div class="space-y-8">
        ...
        <div class="bg-supabase-accent rounded-2xl p-8 text-supabase-dark shadow-2xl shadow-supabase-accent/10 relative overflow-hidden group">
            ...
            <div class="relative z-10">
                <h3 class="text-xl font-black tracking-tighter mb-4">API Secrets</h3>
                <div class="bg-supabase-dark/10 p-4 rounded-xl mb-6 backdrop-blur-sm">
                    <p class="text-[10px] font-bold uppercase tracking-widest opacity-60 mb-2">Active API Key</p>
                ...
                <a href="{{ route('tenant.settings') }}" class="text-xs font-bold underline uppercase tracking-widest">Manage Keys</a>
            </div>
        </div>
        ...
        <div class="bg-supabase-surface border border-supabase-border rounded-2xl p-8">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-black text-white">Webhook</h3>
            ...
            @if($tenant->webhook_url)
                <div class="p-4 bg-supabase-input border border-supabase-border rounded-xl mb-4">
                    <p class="text-[10px] font-bold text-supabase-muted uppercase tracking-widest mb-1">Target URL</p>
