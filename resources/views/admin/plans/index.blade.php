...
<div class="space-y-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
            <h1 class="text-4xl font-bold text-white tracking-normal uppercase">Subscription <span class="text-supabase-accent">Plans</span></h1>
            ...
            <div class="bg-supabase-surface border border-supabase-border rounded-3xl overflow-hidden shadow-2xl hover:border-supabase-accent/30 transition-all group">
                <div class="p-8 space-y-6">
                    <div class="flex items-start justify-between">
                        <div>
                            <h3 class="text-2xl font-bold text-white uppercase tracking-normal">{{ $plan->name }}</h3>
                            <p class="text-[10px] font-bold text-supabase-muted uppercase tracking-wider mt-1">{{ $plan->duration_days }} Days Validity</p>
                        </div>
                        <div class="flex items-center space-x-2 bg-supabase-dark border border-supabase-border px-3 py-1 rounded-full">
                            ...
                            <span class="text-[8px] font-bold uppercase text-white tracking-wider">{{ $plan->is_active ? 'Active' : 'Draft' }}</span>
                        </div>
                    </div>

                    <div class="space-y-1">
                        <p class="text-4xl font-bold text-white leading-none">Rp {{ number_format($plan->price, 0, ',', '.') }}</p>
                        <p class="text-[10px] font-bold text-supabase-muted uppercase tracking-wider">One-time initialization</p>
                    </div>

                    <div class="grid grid-cols-2 gap-4 py-6 border-y border-supabase-border/50">
                        <div>
                            <p class="text-[8px] font-bold text-supabase-muted uppercase tracking-wider mb-1">Channel Capacity</p>
                            <p class="text-sm font-bold text-white">{{ $plan->max_channels }} <span class="text-[10px] text-supabase-muted ml-1 font-bold">Nodes</span></p>
                        </div>
                        <div class="text-right">
                            <p class="text-[8px] font-bold text-supabase-muted uppercase tracking-wider mb-1">Current Usage</p>
                            <p class="text-sm font-bold text-supabase-accent">{{ $plan->tenants_count ?? $plan->tenants()->count() }} <span class="text-[10px] text-supabase-muted ml-1 font-bold">Tenants</span></p>
                        </div>
                    </div>

                    <div class="flex items-center space-x-3 pt-2">
                        <a href="{{ route('admin.plans.edit', $plan) }}" class="flex-1 sb-button-secondary !w-auto !py-3 !text-[10px] !font-bold uppercase tracking-wider text-center">
                            Modify Matrix
                        </a>
        ...
        @empty
            <div class="lg:col-span-3 bg-supabase-surface border border-supabase-border rounded-3xl p-20 text-center relative overflow-hidden shadow-2xl">
                ...
                <div class="relative z-10 max-w-md mx-auto">
                    ...
                    <h3 class="text-2xl font-bold text-white uppercase tracking-normal mb-4">No Subscription Models</h3>
                    <p class="text-supabase-muted text-xs font-bold uppercase tracking-wider leading-relaxed mb-10">Initialize your first subscription plan to start onboarding tenants into the Cekbayar ecosystem.</p>
