...
<div class="mb-12">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
            <h1 class="text-4xl font-black text-white tracking-tight uppercase">Master <span class="text-supabase-accent">Channels</span></h1>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead class="bg-supabase-dark border-b border-supabase-border">
                    <tr>
                        <th class="px-6 py-5 text-[10px] font-black text-supabase-muted uppercase tracking-widest">Channel Name</th>
                        <th class="px-6 py-5 text-[10px] font-black text-supabase-muted uppercase tracking-widest">Code</th>
                        <th class="px-6 py-5 text-[10px] font-black text-supabase-muted uppercase tracking-widest">Base Type</th>
                        <th class="px-6 py-5 text-[10px] font-black text-supabase-muted uppercase tracking-widest text-center">Status</th>
                        <th class="px-6 py-5 text-[10px] font-black text-supabase-muted uppercase tracking-widest text-right">Actions</th>
                    </tr>
                </thead>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-4">
                                    <div class="w-12 h-12 bg-white rounded-xl flex items-center justify-center overflow-hidden border border-supabase-border p-1">
                                        ...
                                        @else
                                            <span class="text-xs font-black text-supabase-dark">{{ substr($channel->name, 0, 2) }}</span>
                                        @endif
                                    </div>
                                    <div>
                                        <p class="text-sm font-black text-white uppercase tracking-tight">{{ $channel->name }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-xs font-black text-supabase-muted tracking-widest">{{ $channel->code }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 bg-supabase-accent/10 text-supabase-accent border border-supabase-accent/20 rounded-lg text-[10px] font-black uppercase tracking-widest">
                                    {{ str_replace('_', ' ', $channel->type) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($channel->is_active)
                                    <span class="px-3 py-1 bg-green-500/10 text-green-500 border border-green-500/20 rounded-lg text-[10px] font-black uppercase tracking-widest">Active</span>
                                @else
                                    <span class="px-3 py-1 bg-supabase-muted/10 text-supabase-muted border border-supabase-muted/20 rounded-lg text-[10px] font-black uppercase tracking-widest">Inactive</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-4">
                                    <a href="{{ route('admin.master-channels.edit', $channel->id) }}" class="text-[10px] font-black text-supabase-accent hover:text-white uppercase tracking-widest transition-colors">Edit</a>
...
@else
    ...
    <div class="bg-supabase-surface border border-supabase-border rounded-3xl p-20 text-center relative overflow-hidden shadow-2xl">
        ...
        <div class="relative z-10 max-w-md mx-auto">
            ...
            <h3 class="text-2xl font-black text-white uppercase tracking-tight mb-4">No Master Channels</h3>
            <p class="text-supabase-muted text-xs font-bold uppercase tracking-widest leading-relaxed mb-10">Add your first master channel to allow tenants to configure it.</p>
