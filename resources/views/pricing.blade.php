        ...
        <div class="relative min-h-screen flex flex-col overflow-hidden">
            ...
            <main class="relative z-10 flex-1 px-6 py-24">
                <div class="max-w-7xl mx-auto text-center mb-24">
                    <h1 class="text-3xl md:text-5xl font-bold text-white tracking-normal leading-tight mb-8">
                        {{ __('Pilih Kapasitas') }} <br><span class="text-supabase-accent">{{ __('Payload Anda') }}</span>.
                    </h1>
                    <p class="max-w-2xl mx-auto text-base md:text-lg text-supabase-muted font-bold tracking-normal leading-relaxed">
                        ...
                        <div class="bg-supabase-surface border border-supabase-border rounded-[40px] p-10 flex flex-col relative overflow-hidden group hover:border-supabase-accent transition-all duration-500 {{ $loop->index === 1 ? 'scale-105 shadow-2xl shadow-supabase-accent/10 border-supabase-accent/50 z-10' : '' }}">
                            @if($loop->index === 1)
                                <div class="absolute top-8 right-8">
                                    <span class="px-3 py-1 bg-supabase-accent text-supabase-dark text-[8px] font-bold uppercase tracking-wider rounded-full">{{ __('Paling Populer') }}</span>
                                </div>
                            @endif

                            <div class="mb-10">
                                <h3 class="text-xs font-bold text-supabase-muted tracking-[0.3em] mb-4">{{ $plan->name }}</h3>
                                <div class="flex items-baseline space-x-2">
                                    <span class="text-3xl md:text-4xl font-bold text-white tracking-normal">Rp {{ number_format($plan->price, 0, ',', '.') }}</span>
                                    <span class="text-xs font-bold text-supabase-muted uppercase tracking-wider">/ {{ $plan->duration_days }} {{ __('Hari') }}</span>
                                </div>
                            </div>
                            ...
                            <div class="space-y-6 mb-12 flex-1">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <p class="text-[10px] font-bold text-white tracking-wider">{{ __('Hingga') }} {{ $plan->max_channels }} {{ __('Node Aktif') }}</p>
                                </div>
                                ...
                                @foreach($features as $feature)
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <p class="text-[10px] font-bold text-supabase-muted tracking-wider">{{ __($feature) }}</p>
                                    </div>
                                @endforeach
                            </div>

                            <a href="{{ route('register', ['plan' => $plan->slug]) }}" 
                               class="w-full py-4 text-center text-[10px] font-bold tracking-wider rounded-2xl transition-all {{ $loop->index === 1 ? 'bg-supabase-accent text-supabase-dark shadow-lg shadow-supabase-accent/20 hover:scale-[1.02]' : 'bg-supabase-input text-white hover:bg-supabase-border' }}">
                                {{ __('Mulai Hubungkan') }}
                            </a>
                        </div>
                    @endforeach
                ...
                <div class="max-w-3xl mx-auto mt-48 text-center">
                    <h2 class="text-2xl font-bold text-white tracking-normal mb-12">{{ __('Tanya Jawab Protokol') }}</h2>
                    <div class="space-y-8 text-left">
                        <div>
                            <h4 class="text-xs font-bold text-white uppercase tracking-wider mb-2">{{ __('Bagaimana cara kerja sinkronisasi Cekbayar?') }}</h4>
                            <p class="text-[11px] text-supabase-muted font-bold tracking-normal leading-relaxed">
                        ...
                        <div>
                            <h4 class="text-xs font-bold text-white uppercase tracking-wider mb-2">{{ __('Apakah dana saya aman?') }}</h4>
                            <p class="text-[11px] text-supabase-muted font-bold tracking-normal leading-relaxed">
